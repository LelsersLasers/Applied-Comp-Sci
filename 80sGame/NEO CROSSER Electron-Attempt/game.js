...
// Replace the built in web prompt (electron does not support)
function createPrompt() {
    smalltalk.prompt('Question', 'How old are you?', '10')
        .then(function (value) {
        console.log(value);
    })["catch"](function () {
        console.log('cancel');
...
function getName(message) {
    var name = localStorage.getItem("NEO CROSSER - Name") != null ? localStorage.getItem("NEO CROSSER - Name") : "";
    console.log("a");
    createPrompt();
    console.log("b");
...
function drawAll() {
    ...
    if (textOpacity > 1 || textOpacity < -opacityDir)
        opacityDir *= -1;
    setDelta();
...
for (var i = 0; i < 10; i++) {
    ...
    var speed = (getRandomInt(700, 900) / 110) * canvas.width * 1 / 6000;
