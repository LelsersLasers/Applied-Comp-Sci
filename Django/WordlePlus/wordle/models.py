...
class Word(models.Model):
    ...
    double_letters = models.BooleanField(
        "Does the word have double letters?", default=False
    )
    common = models.BooleanField("Is it a (sort of) common word?", default=False)

    def __str__(self):
        return "%i: %s (dl: %s, common: %s)" % (
            self.length,
            self.txt,
            self.double_letters,
            self.common,
...
class Score(models.Model):
    ...
    def __str__(self):
        return "%s) %s - %s) '%s' in %s and %s" % (
            self.cup,
            self.sub_date,
            self.account.display_name,
            self.word,
            self.get_guesses_str(),
            self.get_secs_and_mins(),
        )

    def get_rankings_str(self):
        return "%s) '%s' in %s and %s" % (
            self.account.display_name,
            self.word.txt,
            self.get_guesses_str(),
            self.get_secs_and_mins(),
